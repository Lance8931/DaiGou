...
layui.define("layer", function (e) {
	...
	c.prototype.load = function (e) {
		...
		if (!t.isEmptyObject(e)) {
			var debugInfo = "";
			 t.each(e,function(key,value){
				//是否开启调试，开启将会把name value打印出来  
                 if (isDebug) {  
                     // alert("name:"+key+"; value:"+value);  
                     debugInfo += "name:"+key+"; value:"+value+" || ";  
                 }
                 var formField = t.find("[name='"+key+"']");
                 if (t.type(formField[0]) === "undefined") {
                	 if (isDebug) {
                	 ...
                	 if (fieldTagName === "input") {
                		 if (formField[0].attributes["type"].value === "radio") {
                			 t("input:radio[name='"+key+"'][value='"+value+"']").attr("checked","checked");
                		 } else {
                			 formField[0].value = value;
                		 }
